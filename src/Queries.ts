...
const createQuery = ({
    ...
    startDate: string;
    endDate: string;
    vaccineDataElement: string;
    organisationUnits: string[];
    stage: string;
    q: string;
    doseDataElement?: string;
    aggs: { [key: string]: any };
    others?: any[];
}) => {
    let ranger = [
    ...
    let must: any[] = [
        ...
        { term: { [`stage.keyword`]: stage } },

        {
            exists: {
                field: vaccineDataElement,
            },
        },
        ...others,
    ];
    ...
    if (q) {
        must = [
            ...
            {
                match: {
                    [`${vaccineDataElement}_created_by`]:
    ...
    return {
        ...
        query: {
            bool: {
                must: must.filter((f) => f !== undefined),
            },
        },
        aggs,
    };
    ...
    return useQuery<any, Error>(
        ...
        async () => {
            if (startDate && endDate) {
                ...
                //         {
                //             match: {
                //                 bbnyNYD1wgS_created_by:
                //                     String(username).toLowerCase(),
                //             },
                //         },
                //         {
                //             match: {
                //                 LUIsbsm3okG_created_by:
                //                     String(username).toLowerCase(),
                //             },
                //         },
                //         {
...
export function useEs({
    ...
    return useQuery<any, Error>(
        ["es", q, startDate, endDate, ...organisationUnits, stage],
        async () => {
            if (startDate && endDate) {
                const query = createQuery({
                    startDate,
                    ...
                    aggs: {
                        summary: {
                            terms: {
                                field: `${vaccineDataElement}_created_by.keyword`,
                                size: 10000,
                            },
                            aggs: {
                                status: {
...
export function useProgram(programId: string) {
    ...
    const query = {
        program: {
            ...
            params: {
                paging: false,
                fields: "id,name,programTrackedEntityAttributes[id,trackedEntityAttribute[id,name]],programType,trackedEntityType,programStages[id,name,programStageDataElements[id,dataElement[id,name]]]",
            },
        },
    };
...
export async function download(
    ...
    const filename = `Events-${organisations.join(
        "-"
    )}-${startDate}-${endDate}`;
